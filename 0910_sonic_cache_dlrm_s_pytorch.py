...
class my_cache:#embedding table 별로 만들어서 관리...?
    def __init__(self):
        ...
        list_cache_access = []
        for i in range(8):
            #list_cache_weight.append(dict())
            list_cache_access.append(dict())
        #self.list_cache_weight = list_cache_weight
    ...
    def arrange_mini_batch(self, emb_dict_list, cache_keys, sparse_index_group_batch, sparse_offset_group_batch, hbm_keys):#table 또는 HBM, 내부 중복에 있는 key 제외시킴
        sparse_index = sparse_index_group_batch.tolist()
        sparse_offset = sparse_offset_group_batch.tolist()

        #cache_keys = cache에 있는 key들의 list
        mini_batch = dict()
        offset = 1
        for i in sparse_offset:
            if offset == len(sparse_offset):
                mini_batch[i] = sparse_index[i:]
            else :
                mini_batch[i] = sparse_index[i:sparse_offset_group_batch[offset]] #mini_batch = {lS_o:lS_i}
            offset += 1

        del_hbm_keys_mini_batch = dict() #hbm key를 제거한 mini batch를 만들기 위함

        for key, value in mini_batch.items():#hbm에 있는 key 제거
            v_list = []
            for i in value:
                if i not in hbm_keys:
                    v_list.append(i)
            del_hbm_keys_mini_batch[key] = v_list #{ls_o:[ls_i]}

        del_cache_keys_mini_batch = dict()
        #cache에 있는 key 요소 제거 필요
        for key, value in del_hbm_keys_mini_batch.items():
            v_list2 = []
            for i in value:
                if i not in cache_keys:
                    v_list2.append(i)
                del_cache_keys_mini_batch[key] = v_list2

        #sort_mini_batch
        num_access = dict()
        for key, value in del_cache_keys_mini_batch.items():
            sum_access = 0
            for i in value:
                if i in emb_dict_list:
                    sum_access += emb_dict_list[i]
                else:
                    sum_access += 0 #trainig 기준으로 0, 만약 inference 시에 접근하는 횟수도 고려한다면 +1을 해야될 것..?
            num_access[key] = sum_access

        s_num_access = sorted(num_access.items(), key=operator.itemgetter(1), reverse=True) #access(value)를 기준으로 정렬된 mini_batch 결과, type은 list

        sorted_del_cache_keys_mini_batch = dict()

        #access 횟수를 기준으로 {lS_o:lS_i} 정렬
        for key1, val1 in s_num_access:
            for key2, val2 in del_cache_keys_mini_batch.items():
                if key1 == key2:
                    sorted_del_cache_keys_mini_batch[key1] = val2

        #정렬 된 {lS_o:lS_i ,..., lS_o:lS_i}에서 (table과 hbm key를 제거한 상태) 내부적으로 중복되는 key를 한번 더 제거
        test_list = []
        sorted_del_cache_keys_mini_batch2 = sorted_del_cache_keys_mini_batch
        for key1, val1 in sorted_del_cache_keys_mini_batch.items():
            for key2, val2 in sorted_del_cache_keys_mini_batch2.items():
                if key1 == key2:
                    test_list.append(key2)
                    continue
                elif key2 in test_list:
                    continue
                sorted_del_cache_keys_mini_batch[key2] = list(set(val2)-set(val1))

        #sort_mini_batch2, 중복을 완전히 제거한 상태에서 access 횟수를 기록
        num_access2 = dict()

        for key, value in sorted_del_cache_keys_mini_batch.items():
            sum_access2 = 0
            for i in value:
                if i in emb_dict_list: #디버그로 값 확인해볼것.
                    ...
                    sum_access2 += 0
            num_access2[key] = sum_access2

        #중복이 완전히 제거된 dict에서 access를 기준으로 lS_o 정렬, 결과 = {lS_o:sum of access number} 형태
        access_sorted_del_cache_keys_mini_batch = sorted(num_access2.items(), key=operator.itemgetter(1))

        #access 횟수를 기준으로 {lS_o:lS_i} 정렬
        for key1, val1 in access_sorted_del_cache_keys_mini_batch:
            for key2, val2 in sorted_del_cache_keys_mini_batch.items():
                if key1 == key2:
                    sorted_del_cache_keys_mini_batch[key1] = val2

        return sorted_del_cache_keys_mini_batch, access_sorted_del_cache_keys_mini_batch

    def cache_table(self, list_cache_access, sorted_del_cache_keys_mini_batch, access_sorted_del_cache_keys_mini_batch, embedding_table, mean_embedding_table, device):#cache key 관리 및 embedding table weight 수정
        #{lS_o:lS_i} + {lS_o:lS_i} = {lS_i:sum of access number}
        key_and_access = dict()
        for key1, val1 in sorted_del_cache_keys_mini_batch.items():
            for key2, val2 in access_sorted_del_cache_keys_mini_batch:
                if key1 == key2:
                    key_and_access[tuple(val1)] = val2

        #key, weight updates
        if not list_cache_access:
            list_cache_access = key_and_access
            for key, val in list_cache_access.items():
                input = torch.tensor(key).to(device).int()
                offset = torch.tensor([0]).to(device).int()
                mean = mean_embedding_table(input, offset)
                for i in key:
                    embedding_table.weight[i] = mean
        else: #access를 기준으로 key 업데이트 필요.
            for key1, val1 in key_and_access.items():
                for key2, val2 in list_cache_access.items():
                    if val1 > val2:
                        #list_cache_access[key1] = list_cache_access.pop(key2)
                        list_cache_access[key1] = val1
                        list_cache_access.pop(key2)
                        input = torch.tensor(key1).to(device).int()
                        offset = torch.tensor([0]).to(device).int()
                        mean = mean_embedding_table(input, offset)
                        for i in key1:
                            embedding_table.weight[i] = mean
...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        global mean_embedding_table
        embedding_table = nn.ModuleList()
        mean_embedding_table = nn.ModuleList()
        emb_l = nn.ModuleList()
        v_W_l = []
        ...
        for i, embedding_l in enumerate(emb_l):
            m_Emb = nn.EmbeddingBag.from_pretrained(embedding_l.weight, mode = 'mean')
            m_Emb.weight.requires_grad = False
            mean_embedding_table.append(m_Emb)

        return emb_l, v_W_l, embedding_table, mean_embedding_table
    ...
    def __init__(
        ...
        if (
            ...
            # create operators
            if ndevices <= 1:
                self.emb_l, w_list, self.embedding_table, self.mean_embedding_table = self.create_emb(m_spa, ln_emb, weighted_pooling)
    ...
    def apply_emb(self, lS_o, lS_i, v_W_l, embedding_table, mean_embedding_table, emb_dict_list, list_cache_access, device, hbm_keys):
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if self.quantize_emb:
                ...
                if cnt_num < 8:
                    ...
                    c_keys.append(custom_cache.cache_table(list_cache_access[k], sdckmb, asdckmb, E, mean_embedding_table[k], device))
                    cnt_num = cnt_num + 1
                    print(cnt_num)
                else:
                    ...
                    c_keys[k] = custom_cache.cache_table(list_cache_access[k], sdckmb, asdckmb, E, mean_embedding_table[k], device)
                    cnt_num = cnt_num + 1
                    print(cnt_num)
                ...
                #cache_embedding_table에서 data를 먼저 읽음
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i, device):
        ...
        ly = self.apply_emb(lS_o, lS_i, self.v_W_l, embedding_table, mean_embedding_table, emb_dict_list, list_cache_access, device, hbm_keys)
        # for y in ly:
    ...
    def parallel_forward(self, dense_x, lS_o, lS_i):
        ...
        # embeddings
        ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l, self.embedding_table, self.mean_embedding_table)
        # debug prints
        # print(ly)
...
def inference(
    ...
    for i, testBatch in enumerate(test_ld):
        ...
        with open("0910_sonic_test.txt","a") as file:
...
def run():
    ...
    with open('0910_test_access_number.log', 'r') as file_data:
        for i, line in enumerate(file_data):
            if i<8:
                ...
                my_dict = dict(zip(list(map(int, k_list)), v_list))
                emb_dict_list.append(my_dict)

    #hbm에 offloading 되는 key 설정
    global hbm_keys
    hbm_keys = [[0 for j in range(1)] for i in range(8)]
    for a, i in enumerate(emb_dict_list):
        access_keys = list(i.keys())
        access_keys_index = round(len(access_keys)*0.1)
        hbm_keys[a] = access_keys[0:access_keys_index]
    ...
    if use_gpu:
        ...
        if dlrm.ndevices > 1:
            dlrm.emb_l, dlrm.v_W_l, dlrm.embedding_table, dlrm.mean_embedding_table = dlrm.create_emb(
