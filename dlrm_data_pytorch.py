...
class RandomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # multi hot vector에서 click 여부 입력하는 곳
...
def generate_random_output_batch(n, num_targets, round_targets=False):
    ...
    if round_targets:
        ...
        #P = np.ones((n, num_targets)).astype(np.float32)
        #P = np.zeros((n, num_targets)).astype(np.float32)
        P = np.append(np.ones((int(n/2), num_targets)).astype(np.float32),
                      np.zeros((int(n/2), num_targets)).astype(np.float32), axis = 0)
    else:
        print("else")
...
def generate_dist_input_batch(
    ...
    # dense feature
    # multi hot vector dense feature 입력 해 줄 곳
    Xt = torch.tensor(ra.rand(n, m_den).astype(np.float32))
    #Xt = torch.tensor(np.zeros((n, m_den)).astype(np.float32))
    ...
    for size in ln_emb:
        ...
        for _ in range(n):
            ...
            if rand_data_dist == "gaussian":
                ...
                #multi hot dataset 중 embedding 관련 data를 입력해 줄 곳
                #r = ra.random(sparse_group_size)
                #sparse_group_size의 기본값은 현재 100
                a = int(sparse_group_size/2)
                b = int((sparse_group_size-a)/2)
                c = int(sparse_group_size-(a + b))
                r1 = ra.randint(10, size = a)
                r2 = ra.randint(10, 50, size = b)
                r3 = ra.randint(50, 100, size = c)
                sparse_group1 = np.unique(r1.astype(np.int64))
                sparse_group2 = np.unique(r2.astype(np.int64))
                sparse_group3 = np.unique(r3.astype(np.int64))
                sparse_group = np.concatenate((sparse_group1,sparse_group2,sparse_group3), axis=None)
                #sparse_group = np.unique(np.round(r * (size - 1)).astype(np.int64))
            else:
